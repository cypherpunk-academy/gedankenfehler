  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderGrant: (evt, gestureState) => {
        // Calculate initial touch vector from center when gesture starts, accounting for container position
        const containerTop = 600; // Account for container's absolute position
        const touchVectorX = gestureState.x0 - circleCenterX - circleOffsetX;
        const touchVectorY =
          gestureState.y0 - circleCenterY - circleOffsetY - containerTop;
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        // Calculate vector from center to current touch position, accounting for container position
        const containerTop = 600; // Account for container's absolute position
        const touchVectorX = touchX - circleCenterX - circleOffsetX;
        const touchVectorY =
          touchY - circleCenterY - circleOffsetY - containerTop;
        ...
        const angleDiffFromLast = Math.abs(currentAngle - lastAngleRef.current);
        if (
          !crossesBoundary &&
        ...
        // Convert angle to rotation value with improved responsiveness
        const rotationChange = angleDiff / 15; // Increased from 30 to make rotation twice as fast
        const scaleFactor = Math.max(1.5, windowWidth / 800); // Adjusted scale factor for better response
        const newValue =
      ...
      onPanResponderRelease: () => {
        ...
        const newIndex = ((Math.round(rotateValueRef.current) % 12) + 12) % 12;
