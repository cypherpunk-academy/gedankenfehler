...
export default function EinfuehrungScreen() {
  ...
  const showEinfuehrungStory = () => {
    ...
    setCurrentBackground(require("../../assets/images/intro-background.jpg"));
    setActiveIndex(0);
    if (carouselRef.current) {
      // @ts-ignore - scrollTo method exists but TypeScript doesn't recognize it
      carouselRef.current.scrollTo({ index: 0, animated: false });
  ...
  const shouldShowButton =
    (isLastSlide && currentStory === einführungStories) || !isInitialAppLaunch;
      ...
      {shouldShowButton && (
        ...
        >
          <Text style={[styles.buttonText, { fontSize }]}>
            {currentStory === einführungStories ? "Über die App" : "Einführung"}
          </Text>
        </TouchableOpacity>
      )}
